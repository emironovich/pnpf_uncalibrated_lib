...
void generateData(float* X, float* x, float* y, float& f, Matrix3f& R, Vector3f& C, float d) {
	//focal distance
	f = 200 + 1800 * (float)(rand()) / RAND_MAX;

	//rotation
	Vector3f rVec;
	rVec.setRandom();
	rVec /= 2; //???
	Matrix3f rVecSkew = makeSkew(rVec);
	R = rVecSkew.exp();
	...
	Matrix4f XMHom;
	XMHom << XM,
		MatrixXf::Constant(1, 4, 1);
	Matrix<float, 3, 4> pHom = P * XMHom;
