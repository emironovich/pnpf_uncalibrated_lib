...
TestResult runFunction(const char *funcName, Type eps, int it_num = 1) {
  ...
  int succ_num = 0;
  Type succ_threshold = 6e-3; // todo: decide if want to be an argument
  ...
  for (int curr_it = 0; curr_it < it_num; ++curr_it) {
    ...
    if (strcmp(funcName, "p4p") == 0)
      p4p_solver(X, x, y, eps, &solution_num, f_sol_data, f_sol_size,
                 R_sol_data, R_sol_size, T_sol_data, T_sol_size);
    else if (strcmp(funcName, "p3.5p") == 0)
      p35p_solver(X, x, y, eps, &solution_num, f_sol_data, f_sol_size,
                  R_sol_data, R_sol_size, T_sol_data, T_sol_size);
    else {
    ...
    for (int i = 0; i < solution_num; ++i) {
      ...
      Matrix<Type, 3, 1> C = -R.transpose() * T; // todo: fix
